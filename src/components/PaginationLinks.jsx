export default function PaginationLinks({ meta, onPageClick }) {
    ...
    return (
        <div className="flex items-center justify-between border-t border-gray-200 px-4 py-3 mt-4 sm:px-6">
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                ...
                <div>
                    {meta.last_page > 1 &&
                        <nav className="isolate inline-flex -space-x-0 rounded-md shadow-sm" aria-label="Pagination">
                            {meta.links && meta.links.map((link, ind) => (
                                <a
                                    ...
                                    className={"relative z-10 inline-flex items-center border px-4 py-2 text-sm font-medium focus:z-20"
                                        + (ind === 0 ? ' rounded-l-md' : '')
                                        + (link.active ? ' border-indigo-500 bg-indigo-50 text-indigo-600 ' : ' border-gray-300 bg-white text-gray-500 hover:bg-gray-50')
